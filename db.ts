...
import { InsertUser, users, workspaces, projects, projectVersions, generationHistory, usageTracking, deployments, customDomains, templates, marketplaceComponents, projectFiles, billingHistory, apiKeys, projectContexts } from "../drizzle/schema";
...
export async function getProjectFileContent(projectId: string, filePath: string) {
  const db = await getDb();
  if (!db) return null;
  const result = await db.select({ content: projectFiles.content }).from(projectFiles).where(and(eq(projectFiles.projectId, projectId), eq(projectFiles.path, filePath))).limit(1);
...
export async function saveProjectFile(projectId: string, filePath: string, content: string) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  await db.update(projectFiles).set({ content, updatedAt: new Date() }).where(and(eq(projectFiles.projectId, projectId), eq(projectFiles.path, filePath)));
}

export async function createProjectFile(projectId: string, filePath: string, content: string, type: string, language?: string) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
...
// Original createProjectFile (renamed to avoid conflict)
export async function _createProjectFile(projectId: string, name: string, path: string, type: string, content?: string, language?: string) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
...
export async function getProjectContext(projectId: string) {
  const db = await getDb();
  if (!db) return null;
  const result = await db.select().from(projectContexts).where(eq(projectContexts.projectId, projectId)).limit(1);
  return result.length > 0 ? result[0] : null;
}

export async function saveProjectContext(context: { projectId: string; summary: string; structure: any; goals: any[]; lastChanges: any[]; updatedAt: Date; }) {
  const db = await getDb();
  if (!db) return;
  await db.insert(projectContexts)
    .values({ ...context, id: nanoid() })
    .onDuplicateKeyUpdate({ set: { summary: context.summary, structure: context.structure, goals: context.goals, lastChanges: context.lastChanges, updatedAt: new Date() } });
}
